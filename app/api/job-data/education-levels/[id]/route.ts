...
export async function PUT(
  ...
  try {
    ...
    const validation = updateEducationLevelSchema.safeParse(body)
    ...
    if (!result || result.length === 0) {
      return notFoundResponse('Education level not found')
    }

    return successResponse(result[0], false)
  } catch (error: unknown) {
    console.error('Failed to update education level:', error)
    ...
    return errorResponse('Failed to update education level')
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await sql`
      DELETE FROM job_education_levels
      WHERE id = ${id}
    `
    ...
    return errorResponse('Failed to delete education level')
  }
}
