...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `api:v1:pages:${page}:${limit}:${search}:${category}:${tag}:${status}`
    ...
    const countResult = await sql`
      ...
      WHERE p.status = ${status}
        ...
        ${categoryId === 'none' ? sql`AND FALSE` : categoryId ? sql`AND EXISTS (SELECT 1 FROM page_categories WHERE page_id = p.id AND category_id = ${categoryId})` : sql``}
        ${tagId === 'none' ? sql`AND FALSE` : tagId ? sql`AND EXISTS (SELECT 1 FROM page_tags WHERE page_id = p.id AND tag_id = ${tagId})` : sql``}
    `
    ...
    const pages = await sql`
      ...
      WHERE p.status = ${status}
        ...
        ${tagId === 'none' ? sql`AND FALSE` : tagId ? sql`AND EXISTS (SELECT 1 FROM page_tags WHERE page_id = p.id AND tag_id = ${tagId})` : sql``}
      GROUP BY p.id
      ORDER BY p.menu_order ASC, p.publish_date DESC
    ...
    return setCorsHeaders(errorResponse('Failed to fetch pages'), origin)
  }
}
